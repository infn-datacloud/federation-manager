...
from fed_mng.api.utils import change_role, get_users
...
def get_user_list(
    ...
    return get_users(session, item, query)
...
def create_user(
    ...
    is_site_tester: bool = False,
    is_sla_moderator: bool = False,
    is_user_group_manager: bool = False,
    client_credentials: HTTPBasicCredentials = Security(security),
) -> User:
    """GET operation to retrieve all users."""
    try:
    ...
    item = change_role(session, item, Admin, is_admin)
    item = change_role(session, item, SiteAdmin, is_site_admin)
    item = change_role(session, item, SiteTester, is_site_tester)
    item = change_role(session, item, SLAModerator, is_sla_moderator)
    item = change_role(session, item, UserGroupManager, is_user_group_manager)
    return item
